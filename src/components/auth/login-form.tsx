...
const inputClass =
  "w-full rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-card)] px-3.5 py-2.5 text-base text-[var(--text)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--text-muted)]/40 focus:border-[var(--text-muted)]";
...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    setRememberMe(localStorage.getItem(REMEMBER_ME_KEY) === "1");
  }, []);
      ...
      {helpOpen && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center">
          ...
          >
            <h2
              id="login-help-title"
              className="text-lg font-semibold text-[var(--text)] mt-0 mb-3"
            >
              {t("login.helpTitle")}
            </h2>
